  ...
  var defaultOptions = {
    ...
    calcPoints: function(cards) {
      if (_.isArray(cards)) {
        if (cards.length >= 0) {
          var points = 0;
          _.each(cards, card => { points += this._calcSingle(card); });
          return points;
        } else {
          return this._calcSingle(cards[0]);
        }
      } else {
        return this._calcSingle(cards);
      }
    },
     ...
     */
    calcKittyPoints: function(cards, multiplyKitty = 2) {
    ...
    isStronger: function(oldPlay, newPlay, level, trumpSuit, leadingSuit) {
      ...
      var newTrump = newPlay.length == _.filter(newPlay, trumps).length;
      var oldTrump = oldPlay.length == _.filter(oldPlay, trumps).length;
      ...
      var newPlayWithValue = _.map(newPlay, transform);
      var oldPlayWithValue = _.map(oldPlay, transform);

      // Can't win if multiple cards of different suits or different numbers
      var sameCards = _.filter(newPlayWithValue, c => {
    ...
    isAPointCard: function(card) {
      return this._calcSingle(card) > 0;
    },
