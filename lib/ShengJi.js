...
var ShengJi = function(options) {
  ...
  var defaultOptions = {
    ...
    calcPoints: function(cards) {
      if (_.isArray(cards)) {
        if (cards.length > 0) {
          return _.map(cards, card => this._calcSingle(card))
    ...
    isStronger: function(oldPlay, newPlay, level, trumpSuit, leadingSuit) {
      ...
      var newTrump = newPlay.length == _.filter(newPlay, c => {
        return defaultOptions.isTrump(c, level, trumpSuit);
      }).length;
      var oldTrump = oldPlay.length == _.filter(oldPlay, c => {
        return defaultOptions.isTrump(c, level, trumpSuit);
      }).length;
      ...
      var newPlayWithValue = _.map(newPlay, c => { return transform(c); });
      var oldPlayWithValue = _.map(oldPlay, c => { return transform(c); });
